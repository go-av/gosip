...
func (g *GB28181) Invite(ctx context.Context, client server.Client, deviceID string, streamID string, sdp string) (dialog.Dialog, error) {
	...
	return g.server.Invite(ctx, from, to, sdp, func(msg message.Message) {
		msg.SetHeader(message.NewSubjectHeader(fmt.Sprintf("%s:%s,%s:0", deviceID, streamID, client.User())))
	})
}
