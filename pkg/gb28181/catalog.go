...
type CatalogItem struct {
	...
	*/
	...
	Block           string  `xml:"Block"`           // 警 区 (可 选 )
	...
	IPAddres        string  `xml:"IPAddres"`        // 设备 / 区域 / 系统 IP 地址 ( 可选 )
	Port            int     `xml:"Port"`            // 设备/区域/系统端口(可选)
	Pasword         string  `xml:"Pasword"`         // 设备口令(可选)
	Status          string  `xml:"Status"`          // 设备状态 (必选)  ON OFF
	Longitude       float32 `xml:"Longitude"`       // 经度 (可选 )
	Latitude        float32 `xml:"Latitude"`        // 纬度 (可选 )
	PTZType         int     `xml:"PTZType"`         // 摄像机类型扩展,标识摄像机类型:1-球机;2-半球;3-固定枪机;4-遥控枪 机。当目录项为摄像机时可选。
	PositionType    int     `xml:"PositionType"`    // 摄像机位置类型扩展。1-省际检查站、2-党政机关、3-车站码头、4-中心广场 、5-体育场馆 、6-商业中心 、7-宗教场所 、8-校园周边 、9-治安复杂区域 、10-交通干线。当目录项为摄像机时可选。
	RoomType        int     `xml:"RoomType"`        // 摄像机安装位置室外、室内属性。1-室外、2-室内。当目录项为摄像机时可 选 ,缺 省 为 1。
	UseType         int     `xml:"UseType"`         // 摄像机用途属性。1-治安、2-交通、3-重点。当目录项为摄像机时可选。
	SupplyLightType int     `xml:"SupplyLightType"` // 摄像机补光属性。1-无补光、2-红外补光、3-白光补光。当目录项为摄像机 时 可 选 ,缺 省 为 1。
	DirectionType   int     `xml:"DirectionType"`   // 摄像机监视方位属性。1-东、2-西、3-南、4-北、5-东南、6-东北、7-西南、8-西 北。当目录项为摄像机时且为固定摄像机或设置看守位摄像机时可选。
	Resolution      string  `xml:"Resolution"`      // 摄像机支持的分辨率,可有多个分辨率值,各个取值间以 / 分隔。分辨率 取值参见附录F中SDPf字段规定。当目录项为摄像机时可选。
}
...
func (g *GB28181) Catalog(body []byte) (*server.Response, error) {
	...
	cache, ok := g.cache.LoadOrStore(fmt.Sprintf("catalog-%d", cl.SN), &catalogWithCache{})
	cc := cache.(*catalogWithCache)
	if !ok {
		cc.timer = time.NewTimer(10 * time.Second)
		go func() {
			<-cc.timer.C
			g.cache.Delete(fmt.Sprintf("catalog-%d", cl.SN))
		}()
		cc.catalog = cl
	} else {
